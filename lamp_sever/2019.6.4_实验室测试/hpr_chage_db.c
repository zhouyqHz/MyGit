...
//GPS存储数组
char	GPS_Buff_Langitude[50] = "";
char	GPS_Buff_Langitude_int[10] = "";
char	GPS_Buff_Langitude_Char[50] = "";

char	GPS_Buff_Latitude[50]  = "";
char	GPS_Buff_Latitude_int[10] = "";
char	GPS_Buff_Latitude_Char[50] = "";

char    UPDATE_GPS_Langitude[100] = "";
char    UPDATE_GPS_Latitude[100] = "";
//和时间相关
char 	str_t[26] = {0};
struct 	tm* p_time;
...
int processthread(int sockfd)
{
    ...
    while(1)
    {
        ...
        else if(ret>0)
        {
            ...
            else if( (m_receivelength==154)&&(msgbuffer[14]==0x04))
            {
               ...
               // for(i=0; i<81; i++)
                ...
                // printf("%x\r\n",msgbuffer[59]);
                // printf("%x\r\n",msgbuffer[60]);
				bzero(GPS_Buff_Langitude, sizeof(GPS_Buff_Langitude));
				bzero(GPS_Buff_Langitude_int, sizeof(GPS_Buff_Langitude_int));
				bzero(GPS_Buff_Langitude_Char, sizeof(GPS_Buff_Langitude_Char));
				bzero(UPDATE_GPS_Langitude, sizeof(UPDATE_GPS_Langitude));
				
				bzero(GPS_Buff_Latitude_int, sizeof(GPS_Buff_Latitude_int));
				bzero(GPS_Buff_Latitude, sizeof(GPS_Buff_Latitude));
				bzero(GPS_Buff_Latitude_Char, sizeof(GPS_Buff_Latitude_Char));
				bzero(UPDATE_GPS_Latitude, sizeof(UPDATE_GPS_Latitude));
                //获取gps信息
				//经度更新信息生成
				double Langitude_Double = 0;//经度小数部分
				double Langitude_Double_int = 0;//经度整数部分
				strncpy(GPS_Buff_Langitude_int, msgbuffer+128, 2);//获取整数部分字符
				strncpy(GPS_Buff_Langitude, msgbuffer+130, 8);//获取小数部分字符
				
				Langitude_Double_int = atof(GPS_Buff_Langitude_int);//将整数字符转成实数
				Langitude_Double = atof(GPS_Buff_Langitude)/60;//转换并计算小数部分
				
				Langitude_Double = Langitude_Double + Langitude_Double_int;//计算和			
				sprintf(GPS_Buff_Langitude_Char, "%lf", Langitude_Double);//将实数转换成字符
				printf("Langitude_Double_Char:%s\n", GPS_Buff_Langitude_Char);
				//Mysql语句生成											
				strcpy(UPDATE_GPS_Langitude, "UPDATE gps_information SET latitude='");
				strcat(UPDATE_GPS_Langitude, GPS_Buff_Langitude_Char);
				strcat(UPDATE_GPS_Langitude, "';");
				printf("UPDATE_GPS_Langitude:%s\n",UPDATE_GPS_Langitude);
				
				//纬度更新信息生成
				double Latitude_Double = 0;//经度小数部分
				double Latitude_Double_int = 0;//经度整数部分
				strncpy(GPS_Buff_Latitude_int, msgbuffer+141, 3);//获取整数部分字符
				strncpy(GPS_Buff_Latitude, msgbuffer+145, 8);//获取小数部分字符
				
				Latitude_Double_int = atof(GPS_Buff_Latitude_int);//将整数字符转成实数
				Latitude_Double = atof(GPS_Buff_Latitude)/60;//转换并计算小数部分
				
				Latitude_Double = Latitude_Double + Latitude_Double_int;//计算和			
				sprintf(GPS_Buff_Latitude_Char, "%lf", Latitude_Double);//将实数转换成字符
				printf("Langitude_Double_Char:%s\n", GPS_Buff_Latitude_Char);
				//Mysql语句生成											
				strcpy(UPDATE_GPS_Latitude, "UPDATE gps_information SET longitude='");
				strcat(UPDATE_GPS_Latitude, GPS_Buff_Latitude_Char);
				strcat(UPDATE_GPS_Latitude, "';");
				printf("UPDATE_GPS_Latitude:%s\n",UPDATE_GPS_Latitude);
                ...
                if(mysql_main)
                {
					...
					//更新GPS(经度跟为纬度)
					res=mysql_query(mysql_main,UPDATE_GPS_Langitude);
                    printf("res=%d\n",res);
                    if(res)
                    {
                        printf("UPDATE_GPS_Langitudesqlerror\n");
                    }
                    else
                    {
                        printf("UPDATE_GPS_Langitudesqlok\n");
                    }
					res=mysql_query(mysql_main,UPDATE_GPS_Latitude);
                    printf("res=%d\n",res);
                    if(res)
                    {
                        printf("UPDATE_GPS_Latitudesqlerror\n");
                    }
                    else
                    {
                        printf("UPDATE_GPS_Latitudesqlok\n");
                    }
                }
                //mysql_close(mysql_main);
            }
